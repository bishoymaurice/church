...
class SearchChildUI implements Serializable {

	private static final long serialVersionUID = -4797048684314753463L;
	...
	protected VerticalLayout getLayout() {
		...
		nationalIdTextField.setCaption("«·—ﬁ„ «·ﬁÊ„Ì");
		...
		String[] itemNames = new String[] { "«· —»Ì… «·ﬂ‰”Ì…", "ﬁÿ«⁄", "√”—…", "’›", "›’·", "Œ«œ„" };
		...
		itemComboBox.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				else if (itemComboBox.getValue().toString().equals(itemNames[5])) {
					ArrayList<String> result = CommonsHandler.getNamesByType("minister");
					valueComboBox.removeAllItems();
					for (int i = 0; i < result.size(); i++) {
						valueComboBox.addItem(result.get(i));
					}
		...
		Button nationalIdToggleButton = new Button("«·—ﬁ„ «·ﬁÊ„Ì");
		nationalIdToggleButton.setWidth("150px");
		nationalIdToggleButton.addStyleName(ValoTheme.BUTTON_BORDERLESS_COLORED);
		nationalIdToggleButton.addStyleName(ValoTheme.BUTTON_ICON_ALIGN_RIGHT);
		nationalIdToggleButton.addStyleName("button-right-aligned");
		nationalIdToggleButton.addClickListener(new Button.ClickListener() {
			private static final long serialVersionUID = 5569230699947766441L;

			public void buttonClick(ClickEvent event) {
				if (nationalIdToggleButton.getIcon() == null) {
					nationalIdToggleButton.setIcon(FontAwesome.SEARCH_PLUS);
				} else {
					nationalIdToggleButton.setIcon(null);
				}
			}
		});
		searchItemsLayout.addComponent(nationalIdToggleButton, 1, 3);
		...
		Button ministerToggleButton = new Button("«·Œ«œ„");
		ministerToggleButton.setWidth("150px");
		ministerToggleButton.addStyleName(ValoTheme.BUTTON_BORDERLESS_COLORED);
		ministerToggleButton.addStyleName(ValoTheme.BUTTON_ICON_ALIGN_RIGHT);
		ministerToggleButton.addStyleName("button-right-aligned");
		ministerToggleButton.addClickListener(new Button.ClickListener() {
			private static final long serialVersionUID = 5569230699947766441L;

			public void buttonClick(ClickEvent event) {
				if (ministerToggleButton.getIcon() == null) {
					ministerToggleButton.setIcon(FontAwesome.SEARCH_PLUS);
				} else {
					ministerToggleButton.setIcon(null);
				}
			}
		});
		searchItemsLayout.addComponent(ministerToggleButton, 0, 5);
		// ////////////////////////////////////////////////////////////////////////////////////////////////
		...
		submitButton.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if (nationalIdToggleButton.getIcon() == null) {
					selectData.put("nationalId", "0");
				} else {
					selectData.put("nationalId", "1");
				}
				if (eccEducationToggleButton.getIcon() == null) {
				...
				if (ministerToggleButton.getIcon() == null) {
					selectData.put("minister", "0");
				} else {
					selectData.put("minister", "1");
				}

				ArrayList<ArrayList<Object>> searchResult = RequestHandler.handle2DQueryRequest(
						"searchChild", requestData, selectData);
				...
				if (searchResult.size() < 2) {
					...
					searchWindow.addListener(new Window.CloseListener() {
						...
						public void windowClose(CloseEvent e) {
							...
							FileAccess.deleteFile(csvDownloadFilePath);
						}
					});
		...
		cancelButton.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				CommonsUI.clearComponentValue(idTextField, nameTextField, addressNumTextField,
						...
						coursesTextField, skillsTextField, notesTextField, birthdayYearTextField);
				CommonsUI.clearComponentValue(addressStreetComboBox, addressRegionComboBox,
						addressDistrictComboBox, valueComboBox, birthdayDayComboBox, birthdayMonthComboBox);
	...
	private static StreamResource createResource(String fileInputPath, String fileDownloadName) {
