...
public class FamilyOrg implements Serializable {
	...
	private static final long serialVersionUID = 4409150642410592314L;
	...
	public void setSectionOrg(SectionOrg sectionOrg) {
		this.sectionOrg = sectionOrg;
	}

	public SectionOrg getSectionOrg() {
	...
	protected void refreshContainer(String sectionName) {
		container.removeAllItems();

		if (sectionName != null) {
			ArrayList<String> families = OrgHandler.getFamiliesBySection(sectionName);
			...
			for (int i = 0; i < families.size(); i++) {
				...
				if (item != null) {
					item.getItemProperty(header).setValue(families.get(i));
				}
			}
		}

		this.yearOrg.refreshContainer(null);
	}
	...
	public GridLayout getLayout() {
		...
		table.addValueChangeListener(e -> yearOrg.refreshContainer(container.getItem(table.getValue())
				.toString()));
		...
		// ///////////////////////////////////////////////////////////////
		// /////////////////////////////COMMANDS//////////////////////////
		final Command newFamilyCommand = new Command() {
			private static final long serialVersionUID = -4168910857126708902L;

			@Override
			public void menuSelected(final MenuItem selectedItem) {
				if (sectionOrg.getSelectedSection() == null) {
				...
				enterButton.addClickListener(new Button.ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						requestData.put("sectionName", sectionOrg.getSelectedSection());

						int result = RequestHandler.handleUpdateRequest("newFamily", requestData);

						Notification.show(EMapper.getEDesc(result), EMapper.getNType(result));

						refreshContainer(sectionOrg.getSelectedSection());
					}
				});
		...
		// ///////////////////////////////////////////////////////////////
		final Command updateFamilyCommand = new Command() {
			private static final long serialVersionUID = -4168910857126708902L;

			@Override
			public void menuSelected(final MenuItem selectedItem) {
				if (getSelectedFamily() == null) {
				...
				enterButton.addClickListener(new Button.ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						int result = RequestHandler.handleUpdateRequest("updateFamily", requestData);
		...
		// ///////////////////////////////////////////////////////////////
		final Command deleteFamilyCommand = new Command() {
			private static final long serialVersionUID = -4168910857126708902L;
			...
			public void menuSelected(final MenuItem selectedItem) {
				...
				enterButton.addClickListener(new Button.ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						int result = RequestHandler.handleUpdateRequest("deleteFamily", requestData);
		...
		// ///////////////////////////////////////////////////////////////
		final Command assignYearCommand = new Command() {
			private static final long serialVersionUID = -4168910857126708902L;
			...
			public void menuSelected(final MenuItem selectedItem) {
				...
				allNamesComboBox.addFocusListener(new FocusListener() {
					...
					public void focus(FocusEvent event) {
						...
						ArrayList<String> allNames = CommonsHandler.getAllYears();
				...
				enterButton.addClickListener(new Button.ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						HashMap<String, String> requestData = new HashMap<String, String>();
						requestData.put("familyName", container.getItem(table.getValue()).toString());
						requestData.put("yearName", String.valueOf(allNamesComboBox.getValue()));

						int result = RequestHandler.handleUpdateRequest("assignYear", requestData);

						Notification.show(EMapper.getEDesc(result), EMapper.getNType(result));

						yearOrg.refreshContainer(getSelectedFamily());
					}
				});
		...
		final Command downloadReportsCommand = new Command() {
			...
			public void menuSelected(final MenuItem selectedItem) {
				try {
					...
					String[] reportFilesPaths = OrgHandler.generateEftqadReportForFamily(getSelectedFamily(),
							goDate);
		...
		MenuItem addMenuItem = settingItems.addItem(" ÃœÌœ", newFamilyCommand);
		addMenuItem.setIcon(FontAwesome.PLUS_CIRCLE);

		MenuItem editMenuItem = settingItems.addItem("  ⁄œÌ·", updateFamilyCommand);
		editMenuItem.setIcon(FontAwesome.EDIT);

		MenuItem deleteMenuItem = settingItems.addItem(" Õ–›", deleteFamilyCommand);
		deleteMenuItem.setIcon(FontAwesome.TRASH_O);

		settingItems.addSeparator();

		MenuItem assignFamilyMenuItem = settingItems.addItem("  ⁄ÌÌ‰ ’›", assignYearCommand);
