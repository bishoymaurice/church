...
class NewFatherUI implements Serializable {

	private static final long serialVersionUID = -9054842425800447753L;

	protected VerticalLayout getLayout() {
		final VerticalLayout layout = new VerticalLayout();
		layout.setMargin(true);
		layout.setCaption("√» ÃœÌœ");
		...
		submitButton.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				int result = RequestHandler.handleUpdateRequest("newFather", requestData);
				...
				if (result == EMapper.ECODE_SUCCESS) {
					CommonsUI.clearComponentValue(nameTextField, addressNumTextField,
							...
							mobile2TextField, emailTextField, educationTextField,
							eccEducationTextField, coursesTextField, skillsTextField,
							notesTextField);
