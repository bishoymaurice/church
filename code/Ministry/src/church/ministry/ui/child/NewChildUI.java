...
class NewChildUI implements Serializable {
	...
	protected VerticalLayout getLayout() {
		...
		Label status = new Label("«·’Ê—…:");
		layout.addComponent(status);

		MyReceiver receiver = new MyReceiver();
		Upload upload = new Upload(null, receiver);
		upload.setImmediate(true);
		upload.setButtonCaption("≈Œ — «·’Ê—…");
		...
		upload.addListener(new Upload.StartedListener() {
			...
			public void uploadStarted(StartedEvent event) {
				...
				status.setValue("Ã«— ‰ﬁ· «·’Ê—… ... \"" + event.getFilename() + "\"");
			}
		});
		...
		upload.addListener(new Upload.SucceededListener() {
			...
			public void uploadSucceeded(SucceededEvent event) {
				status.setValue(" „ ≈Œ Ì«— «·’Ê—… »‰Ã«Õ:  \"" + event.getFilename() + "\"");
			}
		});
